...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public List<RecurringExpense> getExpiringRecurringExpenses(int daysBefore) {
        ...
        // Thêm Log để kiểm tra giá trị ngày
        Log.d("DB_DEBUG", "Target Date: " + targetDate);

        // Truy vấn dữ liệu KHÔNG có điều kiện userId
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_RECURRING +
                        " WHERE date(" + COLUMN_END_DATE + ") <= date(?)",
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                Log.d("DB_DEBUG", "Found expense: " + name + " - " + endDate);

                // Nếu RecurringExpense có 5 tham số, bỏ userId
                expiringExpenses.add(new RecurringExpense(id, name, amount, startDate, endDate));
            } while (cursor.moveToNext());
        } else {
            Log.d("DB_DEBUG", "No expiring expenses found.");
        }
